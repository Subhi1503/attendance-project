...
class Train:
    def __init__(self,root):
        ...
        img_top = img_top.resize((1000,360),Image.ANTIALIAS)
        self.photoimg_top=ImageTk.PhotoImage(img_top)

        label1 = Label(self.root,image=self.photoimg_top)
        label1.place(x=0,y=40,width=1000,height=360)
        
        b2=Button(self.root,text="TRAIN DATA",command=self.train_classifier,cursor="hand2",font=("time new roman",40,"bold"),bg="grey",fg="white")
        b2.place(x=0,y=400,width=1000,height=50)
        

        img_bottom = Image.open("college_images\penc.jpg")
        img_bottom = img_bottom.resize((1000,500),Image.ANTIALIAS)
        self.photoimg_bottom=ImageTk.PhotoImage(img_bottom)

        label2 = Label(self.root,image=self.photoimg_bottom)
        label2.place(x=0,y=450,width=1000,height=500)
